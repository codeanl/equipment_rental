...
// 生成 JWT
func (j *MyJWT) GenToken(userId int, username string, role []string, uuid string) (string, error) {
	claims := MyClaims{
		UserId:   userId,
		Username: username,
		Role:     role,
...
// 生成 JWT
func (j *MyJWT) GenTokenMember(userId int, phone string) (string, error) {
	claims := MyClaimsMember{
		UserId: userId,
		Phone:  phone,
		RegisteredClaims: jwt.RegisteredClaims{
			Issuer:    config.Cfg.JWT.Issuer,
